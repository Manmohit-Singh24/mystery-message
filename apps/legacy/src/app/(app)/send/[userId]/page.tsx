  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const { data: res } = await axios.get(`/api/get-user?userId=${receiverId}`);
      ...
      <div className=" mb-4">
        <h1 className="text-4xl font-bold">
          {receiver.username ? (
            <>
              Just Say it to{" "}
              <span className="text-indigo-600 dark:text-indigo-400">{receiver.name}</span>
            </>
          ) : (
        ...
        {!receiver.username && (
          <span className="text-destructive">
            Seems like either user not exists or there was an internal error occured
          </span>
        )}

        {receiverId === user?._id && (
          <span className="text-destructive">You can't send message to yourself</span>
        )}
      </div>
        ...
        <CardHeader className="pb-3">
          <div className="flex items-start justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <CardTitle className="text-base font-semibold">{receiver.name}</CardTitle>
                <CardDescription className="text-xs">@{receiver.username}</CardDescription>
              </div>
            </div>
          </div>
          <Separator />
              ...
              <Controller
                ...
                render={({ field }) => (
                  <div className="flex items-center space-x-2">
                    <input {...field} value={receiverId} disabled={true} className="hidden" />
                  </div>
                )}
              />
              <FieldGroup>
                    ...
                    <Field>
                      ...
                      <div className="relative">
                        ...
                        <div className="absolute bottom-1 right-2 text-xs flex items-center gap-1">
                          ...
                          <FieldDescription
                            className={
                              contentCounter < 10 || contentCounter > 500 ? "text-red-500" : ""
                            }
                          >
                            {contentCounter} / 500
                          </FieldDescription>
              ...
              <div className="flex flex-col sm:flex-row gap-5 sm:items-center justify-between mt-4">
                        ...
                        <Field>
                          <div className="flex items-center gap-2 text-nowrap">
                            <Switch
                              ...
                              id="isAnonymous"
                              disabled={
                                user?._id === "guest" || isSending || !receiver.isAcceptingMessage
                              }
                            />
                            <FieldLabel
                              htmlFor="isAnonymous"
                          ...
                          <FieldError>{form.formState.errors.isAnonymous?.message}</FieldError>
                        </Field>
                      )}
                    />
                  </FieldGroup>
                    ...
                    <Controller
                      ...
                      render={({ field }) => (
                        <Field>
                          ...
                          <FieldError>{form.formState.errors.isTrulyAnonymous?.message}</FieldError>
                          {user?._id === "guest" && (
                ...
                <div className="flex items-center">
                  <Button
                    ...
                    disabled={user?._id === receiverId || isSending || !receiver.isAcceptingMessage}
                  >
                    {isSending ? (
                      <>
                        <Spinner /> Sending...
