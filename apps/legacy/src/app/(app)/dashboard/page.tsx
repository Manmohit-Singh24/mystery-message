...
export default function () {
  ...
  const [receivedMessages, setReceivedMessages]: [MessageResType[], any] = useState([]);
  ...
  const fetchSentMessages = useCallback(
    async (isRefresh: boolean = false) => {
      ...
      try {
        ...
        const { data: res } = await axios.get(`api/get-messages?role=sender&cursor=${cursor}`);
  ...
  const fetchReceivedMessages = useCallback(
    async (isRefresh: boolean = false) => {
      ...
      try {
        ...
        const { data: res } = await axios.get(`api/get-messages?role=receiver&cursor=${cursor}`);
  ...
  useEffect(() => {
    const receivedObserver = new IntersectionObserver(
      async (entries) => {
        ...
        if (firstEntry.isIntersecting && !isFetchingMessages && canFetchMoreReceivedMsg)
    ...
    const sentObserver = new IntersectionObserver(
      async (entries) => {
        const firstEntry = entries[0];
        if (firstEntry.isIntersecting && !isFetchingMessages && canFetchMoreSentMsg)
    ...
    if (receivedObserverRef.current) receivedObserver.observe(receivedObserverRef.current);

    if (sentObserverRef.current) sentObserver.observe(sentObserverRef.current);

    return () => {
      if (receivedObserverRef.current) receivedObserver.unobserve(receivedObserverRef.current);

      if (sentObserverRef.current) sentObserver.unobserve(sentObserverRef.current);
    };
  }, [
    isLoading,
    isFetchingMessages,
  ...
  const handleMsgDelete = ({ messageId, role }: { messageId: string; role: string }) => {
    if (role === "sender") {
      setSentMessages((prev: MessageResType[]) =>
    ...
    <>
      ...
      <Tabs defaultValue={currentTab} className="mt-6">
        <TabsList className="py-5 mx-auto bg-foreground dark:bg-foreground/80 rounded-4xl ">
          ...
          <TabsTrigger onClick={() => setCurrentTab("sent")} value="sent" className={tabsClassName}>
            Send
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="sent">
          <div className="flex gap-2 items-center mx-auto w-fit my-2">
            ...
            <RefreshButton callback={fetchSentMessages} isFetchingMessages={isFetchingMessages} />
          </div>
          <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-6">
  ...
  return (
    <Tooltip>
      <TooltipTrigger asChild>
        ...
        >
          <RefreshCcw className={`h-4 w-4 ${isFetchingMessages && "animate-spin"}`} />
        </Button>
      </TooltipTrigger>
  ...
  return (
    <div ref={observerRef} className="flex flex-col items-center">
      ...
      >
        ...
        {canFetchMore ? (isFetchingMessages ? "Loading" : "Load More") : "No More Messages"}
      </Button>
