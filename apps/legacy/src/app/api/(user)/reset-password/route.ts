...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!user || !user.isActivated) return APIResponse(RESPONSES.USER_NOT_FOUND);
    ...
    if (!user.passwordResetToken || !isTokenValid || passwordResetExpiry < new Date())
      return APIResponse(RESPONSES.INVALID_REQUEST(" Invalid or expired password reset link."));
