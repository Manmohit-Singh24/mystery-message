...
export async function POST(req: NextRequest) {
  try {
    ...
    if (user.activationDeadline < new Date()) return APIResponse(RESPONSES.ACCOUNT_NOT_FOUND);

    if (user.activationCode !== code) return APIResponse(RESPONSES.INVALID_TOKEN);
