...
export async function POST(request: NextRequest) {
  try {
    ...
    if (session && session.user._id !== "guest") return APIResponse(RESPONSES.ALREADY_LOGGED_IN);
    ...
    if (existingUserByEmail) {
      ...
      if (existingUserByEmail.isActivated) return APIResponse(RESPONSES.USER_ALREADY_EXISTS_EMAIL);
