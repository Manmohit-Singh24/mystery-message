...
export function AIMessageHelper({ onUse }: { onUse: (message: string) => void }) {
        ...
        {completion && (
          <div className="relative border rounded-md bg-muted/30 p-3">
            <div className="p-4 bg-muted/30 rounded-lg font-medium text-foreground">
              <TypingEffect text={completion} isStreaming={isLoading} speed={20} />
            </div>
...
export function TypingEffect({
  ...
  return (
    <span className="whitespace-pre-wrap">
      {displayed}
      {isStreaming && <span className="w-3 h-3 bg-foreground rounded-full animate-pulse z-10" />}
    </span>
  );
}
