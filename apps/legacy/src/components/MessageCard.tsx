...
export default function MessageCard({
  ...
  role: string;
  handleMsgDelete: ({ messageId, role }: { messageId: string; role: string }) => void;
}) {
  ...
  const handleUnsend = async () => {
    try {
      const { data: res } = await axios.delete(`/api/unsend-message?messageId=${message._id}`);
      if (res.success) {
        toast.success(res.message);
        handleMsgDelete({ messageId: message._id, role });
      } else toast.error(res.message);
        ...
        <CardHeader className="pb-3">
          <div className="flex items-start justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <CardTitle className="text-base font-semibold">{displayUser.name}</CardTitle>
                <CardDescription className="text-xs">@{displayUser.username}</CardDescription>
              </div>
            </div>
      ...
      <AlertDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This action cannot be undone. This will permanently delete the message from your view.
      ...
      <AlertDialog open={showUnsendDialog} onOpenChange={setShowUnsendDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will remove the message for both you and the recipient. This action cannot be
              undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction onClick={handleUnsend} className="bg-orange-600 hover:bg-orange-700">
              Unsend
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
